...
sub mostrarListadoPorEstado{
	...
	if($opciones[0] == 1){
		...
		print GREEN BOLD,"Listado por estado: $estado\n\n", RESET;
			...
			printf BOLD BLUE, "Banco origen: $desde ------------------------------|\n",RESET;
...
sub listarPorPendiente{
	...
	my %desde, %hacia, %estado;
	while ($i <= $#listaHash)  {
		open(ENTRADA, "<$listaHash[$i]");
		my @lineas = <ENTRADA>;
		foreach $linea (@lineas) {
			print $linea;
			($estado = $linea) =~ s/^[^;]*;[^;]*;[^;]*;[^;]*;[^;]*;[^;]*;[^;]*;([^;]*);.*$/\1;\2/g;
			print split(/;/, $estado) eq "pendiente"; 
			if (split(/;/, $estado) eq "pendiente"){
				print 'pendiente';
				($desde = $linea) =~ s/^[^;]*;([^;]*);[^;]*;[^;]*;[^;]*;[^;]*;([^;]*);.*$/\1;\2/g;
				($hacia = $linea) =~ s/^[^;]*;[^;]*;[^;]*;([^;]*);[^;]*;[^;]*;([^;]*);.*$/\1;\2/g;
